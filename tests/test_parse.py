...
if args.mode[0] == "run":
    ...
    for target in targets:
        try:
            with codecs.open(os.path.join("tests/data", target), "r") as f:
                data = f.read().split("\n--\n")
        except IOError as e:
            sys.stderr.write(
            ...
            try:
                ...
                with codecs.open(os.path.join("tests/data", target), "r", 'cp1252') as f:
                    data = f.read().split("\n--\n")
            except UnicodeDecodeError as e:
                # Fall back to trying all registered codecs
                data = read_encoded_file(os.path.join("tests/data", target)).split("\n--\n")
        try:
            with codecs.open(os.path.join("tests/target_default", target), "r") as f:
                default = f.read()
            with codecs.open(os.path.join("tests/target_normalized", target), "r") as f:
                normalized = f.read()
        except IOError as e:
            sys.stderr.write(
                "Missing target data for domain %(domain)s. Run `./test_parse.py update %(domain)s` to correct this, "
                "after verifying that wizard_domaininfo can correctly parse this particular domain.\n" % {
                    "domain": target})
            errors = True
            continue
    ...
    for target, data, target_default, target_normalized in suites:
        for normalization in (True, []):
            ...
            parsed = wizard_domaininfo.parse.parse_raw_whois(data, normalized=normalization)
    ...
    for target, data in updates:
        default = wizard_domaininfo.parse.parse_raw_whois(data)
        normalized = wizard_domaininfo.parse.parse_raw_whois(data, normalized=True)
        with codecs.open(os.path.join("tests/target_default", target), "w") as f:
            f.write(encoded_json_dumps(default))
        with codecs.open(os.path.join("tests/target_normalized", target), "w") as f:
