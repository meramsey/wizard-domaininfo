...
parser = argparse.ArgumentParser(description="Retrieves and parses WHOIS data for a domain name using "
                                             "wizard_domaininfo.")
parser.add_argument("-r", "--raw", action="store_true",
                    help="Outputs raw WHOIS data and doesn't attempt to parse it. Segments are separated by a "
                         "double-dash (--).")
parser.add_argument("-j", "--json", action="store_true",
                    help="Outputs structured WHOIS data in JSON format, according to the wizard_domaininfo API.")
parser.add_argument("-f", "--file", action="store",
                    help="Loads and parses raw double-dash-delimited WHOIS data from a specified file, instead of "
                         "contacting a WHOIS server.",
                    default=None)
...
if args.file is None:
    data, server_list = wizard_domaininfo.net.get_whois_raw(args.domain[0], with_server_list=True)
else:
    server_list = []
